...
public class Game extends JPanel implements NewFrame
{
    ...
    private Move moveOther = new Move();
     ...
     */
    public void cardPicked( int moveFromId )
    {
        if ( root != null )
        {
    ...
    /**Checks and makes the move for the card clicked.
     * @param num int
     */ 
    public void deal( int num )
    {
        for( int i = 0; i < 4; i++ )
        {
            if( num == -1 )
            {
                moveOther.draw( _drawPile, _play[ i ] );  
                reload();
            }
            else
            {
                if(  !_play[num].getStack().isEmpty() 
                       && !_play[i].getStack().isEmpty() )
                {
                    if( _play[num].peek().getSuit() 
                           == _play[i].peek().getSuit() )
                    {
                        if( _play[num].peek().compareTo( _play[i].peek() ) < 0 )
                        {
                            moveOther.discard( _play[num], _discards  );
                            reload();
                            break;
                        }
                    }
                ...
                if ( _play[num].getStack().size() > 1 )
                {
                    if ( !_play[num].getStack().isEmpty() 
                            && _play[i].getStack().isEmpty() )
                    {
                        moveOther.toEmpty( _play[num], _play[i] );
                        reload();
                        break;
                    }
                }
    ...
    private String makeAMove()
    {
        ...
        // IT WORKS BUT IT IS NOT SMART
        for( int i = 0; i < 4; i++ )
        {
            if( _play[ 0 ].getStack().size() == 0 &&
                _play[ 1 ].getStack().size() == 0 &&
                _play[ 2 ].getStack().size() == 0 &&
                _play[ 3 ].getStack().size() == 0 )
            {
                moveOther.draw( _drawPile, _play[i] );
                reload();
            }
            else if( _drawPile.getStack().size() == 0 )
            {
                msg = "Pile Empty";
