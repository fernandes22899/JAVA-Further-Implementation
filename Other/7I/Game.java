...
public class Game extends JPanel implements NewFrame
{
    ...
    private void cardPicked( int moveFromId )
    {
        ...
        if( moveFromId == 0 )
            draw();
        else if( moveFromId > 0 && moveFromId < 5 )
            discard( moveFromId );
    ...
    /**Method that discards card.
     * @param count int
     */
    public void discard( int count )
    {
        //--Makes card stacks from all the stacks using getStack() method--
        CardStack cs1 = _play[ 0 ].getStack();
        CardStack cs2 = _play[ 1 ].getStack();
        CardStack cs3 = _play[ 2 ].getStack();
        CardStack cs4 = _play[ 3 ].getStack();
        CardStack usedDeck = _discards.getStack();
            
        //--Switch case goes through all stacks and checks cards--
        switch( count )
        {
            //--Beginning of 1st card deck--
            case 1:
                if( cs1.peek().getSuit().ordinal() 
                       == cs2.peek().getSuit().ordinal() )
                    if( cs1.peek().getRank().ordinal() 
                        < cs2.peek().getRank().ordinal() )
                    {
                        usedDeck.push( cs1.pop() );
                        usedDeck.displayCards( -1, true );
                        update();
                    }   
                
                if( cs1.isEmpty() == false && cs1.peek().getSuit().ordinal() 
                       == cs3.peek().getSuit().ordinal() )
                    if( cs1.peek().getRank().ordinal() 
                           < cs3.peek().getRank().ordinal() )
                    {
                        usedDeck.push( cs1.pop() );
                        usedDeck.displayCards( -1, true );
                        update();
                ...
                if( cs1.isEmpty() == false && cs1.peek().getSuit().ordinal() 
                       == cs4.peek().getSuit().ordinal() )
                    if( cs1.peek().getRank().ordinal() 
                           < cs4.peek().getRank().ordinal() )
                    {
                        usedDeck.push( cs1.pop() );
                        usedDeck.displayCards( -1, true );
                        update();
                ...
                //--Beginning of 2nd card deck--
            case 2:    
                
                if( cs2.peek().getSuit().ordinal() 
                       == cs3.peek().getSuit().ordinal() )
                    if( cs2.peek().getRank().ordinal() 
                        < cs3.peek().getRank().ordinal() )
                    {
                        usedDeck.push( cs2.pop() );
                        usedDeck.displayCards( -1, true );
                        update();
                    }   
                
                if( cs2.isEmpty() == false && cs2.peek().getSuit().ordinal() 
                       == cs4.peek().getSuit().ordinal() )
                    if( cs2.peek().getRank().ordinal() 
                           < cs4.peek().getRank().ordinal() )
                    {
                ...
                if( cs2.isEmpty() == false && cs1.peek().getSuit().ordinal() 
                       == cs1.peek().getSuit().ordinal() )
                    if( cs2.peek().getRank().ordinal() 
                           < cs1.peek().getRank().ordinal() )
                    {
                        usedDeck.push( cs2.pop() );
                        usedDeck.displayCards( -1, true );
                        update();
                ...
                //--Beginning of 3rd card deck--
            case 3:
                if( cs3.peek().getSuit().ordinal() 
                       == cs4.peek().getSuit().ordinal() )
                    if( cs3.peek().getRank().ordinal() 
                           < cs4.peek().getRank().ordinal() )
                    {
                        usedDeck.push( cs3.pop() );
                        usedDeck.displayCards( -1, true );
                        update();
                    }   
                
                if( cs3.isEmpty() == false && cs3.peek().getSuit().ordinal() 
                       == cs1.peek().getSuit().ordinal() )
                    if( cs3.peek().getRank().ordinal() 
                           < cs1.peek().getRank().ordinal() )
                    {
                ...
                if( cs3.isEmpty() == false && cs1.peek().getSuit().ordinal() 
                       == cs2.peek().getSuit().ordinal() )
                    if( cs3.peek().getRank().ordinal() 
                           < cs2.peek().getRank().ordinal() )
                    {
                ...
                //--Beginning of 4th card deck--
            case 4:
                if( cs4.peek().getSuit().ordinal() 
                       == cs1.peek().getSuit().ordinal() )
                    if( cs4.peek().getRank().ordinal() 
                           < cs1.peek().getRank().ordinal() )
                    {
                        usedDeck.push( cs4.pop() );
                        usedDeck.displayCards( -1, true );
                        update();
                    }   
                
                if( cs4.isEmpty() == false && cs4.peek().getSuit().ordinal() 
                       == cs2.peek().getSuit().ordinal() )
                    if( cs4.peek().getRank().ordinal() 
                           < cs2.peek().getRank().ordinal() )
                    {
    ...
    /**draw method draws cards.
     * @return boolean
     */
    public boolean draw()
    {
        CardStack card = _drawPile.getStack();
        ...
        for( int i = 0; i < 4; i++ )
        {
            CardStack cStack = _play[ i ].getStack();
            
            if( _play[ i ].getStack() == null )
                return false;
            
            cStack.push( card.pop() );
            cStack.displayCards( -1, true );    
        }
        
        return true;
    }
    
    /**Method moves card to empty stack.
     */ 
    public void emptyMove()
    {
        CardStack[] stack = new CardStack[ 4 ];
        int[] stackDepth = new int[ 4 ];
        
        for( int i = 0; i < 4; i++ )
        {
            stack[ i ] = _play[ i ].getStack();
            stackDepth[ i ] = _play[ i ].getStack().size();
        }
        
        int largest = findLarge( stackDepth );
        
        for( CardStack pile: stack )
            if( pile.isEmpty() )
            {
                pile.push( stack[ largest ].pop() );
                break;
            }
    }
    
    /**Helper Method that helps find largest pile size.
     * @param pack int[]
     * @return int
     */
    public int findLarge( int[] pack )
    {
        //--Finds largest deck--
        int help = 0;
        int stack = pack[0];
