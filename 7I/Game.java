...
public class Game extends JPanel implements NewFrame
{
    ...
    private void cardPicked( int moveFromId )
    {
        ...
        if( moveFromId == 1 || moveFromId == 2 || moveFromId == 3 ||
           moveFromId == 4 )
        {
            m = new Move();
            updateLabels( null );
    ...
    //         user play mode
    //         ad hoc program mode
    //         tree program mode ( 7P )
    //   2. Creating GameState objects
    //   3. Determining what moves are possible given the current state
    //   4. Building the tree; this is most easily done recursively. 7P
    //   5. Accessing the tree and state information. 7P
    //   6. Utility methods to keep others short.
    /////////////////////////////////////////////////////////////////////
    
    public boolean canDiscard( int index ) //If able to discard
    {
        Pile p = _play[ index - 1 ];
        if( p == null )
            return false;
        
        Card c = p.getStack().peek();
        if( c == null )
            return false;
    ...
    public boolean canMove( int move ) //3. determine possible moves
    {
        Move m = new Move();
        return true;
    }
